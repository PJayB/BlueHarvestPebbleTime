...
#ifndef FIXMATH_NO_FLOAT
static inline float   fix16_to_float(fix16_t a) { return (float)a / fix16_one; }
static inline double  fix16_to_dbl(fix16_t a)   { return (double)a / fix16_one; }
#endif

// @PJayB: split to_int into two functions
static inline int16_t fix16_to_int_floor(fix16_t a)
{
    return (a >> 16);
}
static inline int16_t fix16_to_int_round(fix16_t a)
{
    if (a >= 0)
        return (int16_t)((a + (fix16_one >> 1)) / fix16_one);
    return (int16_t)((a - (fix16_one >> 1)) / fix16_one);
}
...
// @PJayB: return the fractional part
static inline fix16_t fix16_frac(fix16_t x)
{
    if (x < 0)
        return x - fix16_ceil(x);
    else
        return x - fix16_floor(x);
}

// @PJayB: return the unsigned fractional part
static inline fix16_t fix16_ufrac(fix16_t x)
{
    return (x & 0x0000FFFFU);
}

// @PJayB: round to nearest integer
static inline fix16_t fix16_round(fix16_t x)
{
    if (x & 0x8000UL)
        return fix16_ceil(x);
    else
        return fix16_floor(x);
}
