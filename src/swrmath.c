...
void matrix_vector_transform(vec3* V, const matrix* M) {
    ...
    fix16_t fW = 
        fix16_add(fix16_mul(M->m[0][3], V->v[0]),
            fix16_add(fix16_mul(M->m[1][3], V->v[1]), 
                fix16_add(fix16_mul(M->m[2][3], V->v[2]),
                    M->m[3][3])));
    ...
    fix16_t fX0 = fix16_mul(fix16_mul(M->m[0][0], V->v[0]), iW);
    fix16_t fX1 = fix16_mul(fix16_mul(M->m[1][0], V->v[1]), iW);
    fix16_t fX2 = fix16_mul(fix16_mul(M->m[2][0], V->v[2]), iW);
    ...
    fix16_t fY0 = fix16_mul(fix16_mul(M->m[0][1], V->v[0]), iW);
    fix16_t fY1 = fix16_mul(fix16_mul(M->m[1][1], V->v[1]), iW);
    fix16_t fY2 = fix16_mul(fix16_mul(M->m[2][1], V->v[2]), iW);
    ...
    fix16_t fZ0 = fix16_mul(M->m[0][2], V->v[0]);
    fix16_t fZ1 = fix16_mul(M->m[1][2], V->v[1]);
    fix16_t fZ2 = fix16_mul(M->m[2][2], V->v[2]);
    fix16_t fZ3 = M->m[3][2];
    ...
    V->v[0] = fX;
    V->v[1] = fY;
    V->v[2] = fix16_div(fW, fZ); // Invert Z
}
